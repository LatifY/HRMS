...
public interface JobAdvertisementDao extends JpaRepository<JobAdvertisement, Integer> {
    ...
    @Query("Select j from latifyilmaz.hrms.entities.concretes.JobAdvertisement j where " +
            ...
            "((:#{#filter.minSalary}) IS NULL OR ((:#{#filter.minSalary}) IS 0 OR j.minSalary >= (:#{#filter.minSalary})) and " +
            "((:#{#filter.maxSalary}) IS NULL OR ((:#{#filter.maxSalary}) IS 0 OR j.maxSalary <= (:#{#filter.maxSalary}))")
